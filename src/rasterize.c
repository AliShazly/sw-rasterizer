...
static void gl_projection_mat(mat4x4 ret, double t, double b, double l,
                              double r, double n, double f);
static void view_frustum_bounds(double out_bounds[4], double angle_of_view,
                                double aspect, double z);
static void world_to_clip(vec4 dst, vec4 world_pt, mat4x4 view_mat,
                          double aspect, double fov_deg, double near,
                          double far);
static void clip_space_to_screen(vec4 out_screen_pt, vec4 clip_space_pt,
                                 int rows, int cols);
static enum ClipFlag clip_triangle_near(vec4 triangle[TRI_NPTS],
                                        vec4 out_pts[TRI_NPTS * 2],
                                        double near_clip);
...
void lookat(vec3 camera_pos, vec3 target, mat4x4 dst, vec3 up) {
    ...
    mat4x4 orientation = {{x_axis[0], y_axis[0], z_axis[0], 0},
...
static void world_to_clip(vec4 dst, vec4 world_pt, mat4x4 view_mat,
    ...
    // getting frustum bounds at -near, which is the near clip plane in front of
    // the camera.
    view_frustum_bounds(fbounds, fov_deg, aspect, -near);
    gl_projection_mat(proj_mat, fbounds[0], fbounds[1], fbounds[2], fbounds[3],
                      near, far);
...
static void clip_space_to_screen(vec4 out_screen_pt, vec4 clip_space_pt,
    ...
    out_screen_pt[3] =
        1 / clip_space_pt[3]; // depth inverse for persp correct interpolation
}
...
static enum ClipFlag clip_triangle_near(vec4 triangle[TRI_NPTS],
    ...
    // 2 intersections between tri and clipping plane should be guaranteed from
    // here
    ...
    // When idx 1 is the only clipped point, the triangulated quad gets created
    // with a clockwise (backwards) winding order I'm not sure if this is always
    // the case, but flipping it based on this seems to work.
    bool quad_flip =
        (clipped_quad) &&
        ((int)(ptr_dist(triangle[0], clipped_pts[0]) / sizeof(vec4)) == 1);
    ...
    for (int i = 0; i < n_unclipped_pts; i++) {
        for (int j = 0; j < n_clipped_pts; j++) {
            ...
            plane_line_intersection(intersect, *unclipped_pts[i],
                                    *clipped_pts[j], plane_pt, plane_normal);

            if (clipped_quad) {
                // first triangle  (i==0) -> {intersect, unclipped[i],
                // unclipped[i + 1]} second triangle (i==1) -> {unclipped[i],
                // intersect, prev_intersect}

                if (i == 0) {
                    memcpy(out_pts[0], intersect, sizeof(vec4));
                    memcpy(out_pts[quad_flip ? 2 : 1], *unclipped_pts[i],
                           sizeof(vec4));
                    memcpy(out_pts[quad_flip ? 1 : 2], *unclipped_pts[i + 1],
                           sizeof(vec4));
                } else // i == 1
                {
                    memcpy(out_pts[3], *unclipped_pts[i], sizeof(vec4));
                    memcpy(out_pts[quad_flip ? 5 : 4], intersect, sizeof(vec4));
                    memcpy(out_pts[quad_flip ? 4 : 5], out_pts[0],
                           sizeof(vec4));
                }

            } else // clipped_tri
            {
                // intersected point inherits it's clipped idx to keep winding
                // order consistent
                int orig_idx =
                    ptr_dist(triangle[0], clipped_pts[j]) / sizeof(vec4);
        ...
        if (!clipped_quad) {
            int orig_idx =
                ptr_dist(triangle[0], unclipped_pts[i]) / sizeof(vec4);
...
void draw_object(RenderCtx *ctx) {
    ...
    for (int i = 0; i < ctx->mesh->size; i += TRI_NPTS) {
        ...
        vec4 clipped_pts[TRI_NPTS *
                         2]; // clipped tri gets split into max 2 other tris

        for (int j = 0; j < TRI_NPTS; j++) {
            vec4 rotated_pt = {0, 0, 0, 1};
            vec3 norm_rot_axis = {0, 1, 0};
            rotate_point(rotated_pt, norm_rot_axis, ctx->mesh->verts[i + j],
                         time);

            world_to_clip(clip_space_tri[j], rotated_pt, ctx->view_mat, aspect,
        ...
        enum ClipFlag flag =
            clip_triangle_near(clip_space_tri, clipped_pts, NEAR_CLIP);

        switch (flag) {
        case UNCLIPPED: {
            vec4 screen_tri[TRI_NPTS];
            for (int i = 0; i < TRI_NPTS; i++) {
                clip_space_to_screen(screen_tri[i], clip_space_tri[i],
                                     ctx->rows, ctx->cols);
            }
        ...
        case CLIPPED_TRI: {
            vec4 screen_tri[TRI_NPTS];
            for (int i = 0; i < TRI_NPTS; i++) {
                clip_space_to_screen(screen_tri[i], clipped_pts[i], ctx->rows,
                                     ctx->cols);
            }
        ...
        case CLIPPED_QUAD: {
            for (int i = 0; i < TRI_NPTS * 2; i += TRI_NPTS) {
                vec4 screen_tri[TRI_NPTS];
                for (int j = 0; j < TRI_NPTS; j++) {
                    clip_space_to_screen(screen_tri[j], clipped_pts[i + j],
                                         ctx->rows, ctx->cols);
                }
...
void draw_object_wireframe(RenderCtx *ctx) {
    ...
    /*         world_to_screen(screen_triangle[j], view_mat, rotated_pt,
     * ctx->rows, ctx->cols); */
    /*     } */

    /*     color_t color = {102, 1, 19}; */
    /*     draw_line(screen_triangle[0][0], screen_triangle[0][1],
     * screen_triangle[1][0], screen_triangle[1][1], */
    /*             color, ctx); */
    /*     draw_line(screen_triangle[1][0], screen_triangle[1][1],
     * screen_triangle[2][0], screen_triangle[2][1], */
    /*             color, ctx); */
    /*     draw_line(screen_triangle[2][0], screen_triangle[2][1],
     * screen_triangle[0][0], screen_triangle[0][1], */
    /*             color, ctx); */
    /* } */
}
...
vec2 **compute_grid(double size, int n_subdivs, int *out_rows, int *out_cols) {
    ...
    vec2(*out_subd_lines)[subd_line_len] =
        malloc(sizeof(vec2) * subd_line_len * n_lines);
...
void draw_grid(RenderCtx *ctx) {
    ...
    /* vec2 (*grid_pts_2d)[ctx->grid_cols] = (vec2
     * (*)[ctx->grid_cols])ctx->grid_points; */
    ...
    /*         world_to_screen(screen_pts[j], view_mat, world_pt, ctx->rows,
     * ctx->cols); */
    /*     } */
    ...
    /*     draw_line(screen_pts[0][0], screen_pts[0][1], screen_pts[2][0],
     * screen_pts[2][1], color, ctx); */
    /*     draw_line(screen_pts[1][0], screen_pts[1][1], screen_pts[3][0],
     * screen_pts[3][1], color, ctx); */
    /* } */
}
