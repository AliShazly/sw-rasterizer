...
LINMATH_H_FUNC void vec##n##_scale(vec##n r, vec##n const v, double const s) \
{ \
	int i; \
	for(i=0; i<n; ++i) \
		r[i] = v[i] * s; \
...
LINMATH_H_FUNC double vec##n##_len(vec##n const v) \
{ \
	return sqrtf(vec##n##_mul_inner(v,v)); \
} \
LINMATH_H_FUNC void vec##n##_norm(vec##n r, vec##n const v) \
{ \
	double k = 1.0 / vec##n##_len(v); \
	vec##n##_scale(r, v, k); \
} \
...
LINMATH_H_FUNC void mat4x4_scale(mat4x4 M, mat4x4 a, double k)
{
	int i;
	for(i=0; i<4; ++i)
		vec4_scale(M[i], a[i], k);
}
LINMATH_H_FUNC void mat4x4_scale_aniso(mat4x4 M, mat4x4 a, double x, double y, double z)
{
	int i;
...
LINMATH_H_FUNC void mat4x4_translate(mat4x4 T, double x, double y, double z)
{
	mat4x4_identity(T);
	T[3][0] = x;
	T[3][1] = y;
...
LINMATH_H_FUNC void mat4x4_translate_in_place(mat4x4 M, double x, double y, double z)
{
	vec4 t = {x, y, z, 0};
	vec4 r;
	int i;
...
LINMATH_H_FUNC void mat4x4_rotate(mat4x4 R, mat4x4 M, double x, double y, double z, double angle)
{
	double s = sinf(angle);
	double c = cosf(angle);
	vec3 u = {x, y, z};
...
LINMATH_H_FUNC void mat4x4_rotate_X(mat4x4 Q, mat4x4 M, double angle)
{
	double s = sinf(angle);
	double c = cosf(angle);
	mat4x4 R = {
...
LINMATH_H_FUNC void mat4x4_rotate_Y(mat4x4 Q, mat4x4 M, double angle)
{
	double s = sinf(angle);
	double c = cosf(angle);
	mat4x4 R = {
...
LINMATH_H_FUNC void mat4x4_rotate_Z(mat4x4 Q, mat4x4 M, double angle)
{
	double s = sinf(angle);
	double c = cosf(angle);
	mat4x4 R = {
...
LINMATH_H_FUNC void mat4x4_frustum(mat4x4 M, double l, double r, double b, double t, double n, double f)
{
	M[0][0] = 2.f*n/(r-l);
...
LINMATH_H_FUNC void mat4x4_ortho(mat4x4 M, double l, double r, double b, double t, double n, double f)
{
	M[0][0] = 2.f/(r-l);
	M[0][1] = M[0][2] = M[0][3] = 0.f;
...
LINMATH_H_FUNC void mat4x4_perspective(mat4x4 m, double y_fov, double aspect, double n, double f)
{
...
LINMATH_H_FUNC void quat_scale(quat r, quat v, double s)
{
	int i;
	for(i=0; i<4; ++i)
		r[i] = v[i] * s;
}
LINMATH_H_FUNC double quat_inner_product(quat a, quat b)
{
	double p = 0.f;
	int i;
	for(i=0; i<4; ++i)
...
LINMATH_H_FUNC void quat_rotate(quat r, double angle, vec3 axis) {
	vec3 v;
...
LINMATH_H_FUNC void mat4x4_arcball(mat4x4 R, mat4x4 M, vec2 _a, vec2 _b, double s)
{
	...
	double const angle = acos(vec3_mul_inner(a_, b_)) * s;
